        ...
        for j in range(1, min(6, len(frag_lines))):
            ...
            if c:
                ...
                if unit == 'ms':
                    ...
                    frag_cpu = value * 1000.0
                elif unit == 'm':
                    frag_cpu = value * 60.0 * 1000.0
                elif unit == 'h':
                    frag_cpu = value * 3600.0 * 1000.0
                elif unit == 'ns':
                    frag_cpu = value / 1_000_000.0      # 纳秒 → 毫秒
                elif unit == 'us':
                    frag_cpy = value / 1_000.0          # 微秒 → 毫秒
                else:
                    frag_cpu = None
        if debug:
...
def main():
    ...
    parser.add_argument('--server', default='localhost:8080', help='Presto server address (host:port)')
    ...
    with open(args.output, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow(['query_id', 'plan_id', 'operator_type', 'child_plan',
                         'l_input_rows', 'r_input_rows', 'output_rows',
                         'execution_time', 'query_dop', 'peak_mem', 'dop'])

        for idx, sql in enumerate(sqls, start=1):
            print(f"Processing query {idx}/{len(sqls)}: {sql[:80]}...")
            explain_sql = f"EXPLAIN ANALYZE {sql}"
            try:
                output = run_presto_query(args.presto_jar, args.server, args.catalog, args.schema, explain_sql)
                ...
                for op in operators:
                    writer.writerow([
                        ...
                        op['output_rows'],
                        op['cpu_time_ms'],
                        64,  # query_dop placeholder
                        '',  # peak_mem placeholder
                        64   # dop placeholder
                    ])
            except Exception as e:
